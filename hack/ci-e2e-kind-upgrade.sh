...
# version' folder back to the 'next version' folder. The virtual garden kubeconfig is generated by `make gardener-up`
# after the virtual garden cluster has been deployed. As `make gardener-up` is executed from the 'previous version'
# folder during `install_previous_release`, the kubeconfig is written there. However, the upgrade tests are executed
# from the 'next version' folder and require the kubeconfig at that location.
function copy_virtual_garden_kubeconfig_from_old_gardener_version_folder() {
...
KUBECONFIG=$KUBECONFIG_VIRTUAL_GARDEN_CLUSTER kubectl wait seed $SEED_NAME --timeout=5m --for=jsonpath="{.status.gardener.version}=$GARDENER_NEXT_RELEASE"
...
KUBECONFIG=$KUBECONFIG_VIRTUAL_GARDEN_CLUSTER TIMEOUT=1200 ./hack/usage/wait-for.sh seed "$SEED_NAME" GardenletReady SeedSystemComponentsHealthy ExtensionsReady BackupBucketsReady
