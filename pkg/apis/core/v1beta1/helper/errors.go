...
var (
	unauthenticatedRegexp               = regexp.MustCompile(`(?i)(Unauthorized|InvalidAuthenticationTokenTenant|Authentication failed|AuthFailure)`)
	unauthorizedRegexp                  = regexp.MustCompile(`(?i)(InvalidClientTokenId|SignatureDoesNotMatch|AuthorizationFailed|invalid character|invalid_grant|invalid_client|Authorization Profile was not found|cannot fetch token|no active subscriptions|InvalidAccessKeyId|InvalidSecretAccessKey|query returned no results|UnauthorizedOperation|not authorized|InvalidSubscriptionId|AccessDenied|OperationNotAllowed|Error 403)`)
