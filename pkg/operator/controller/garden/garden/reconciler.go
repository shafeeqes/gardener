...
func (r *Reconciler) getGlobalObservabilitySecret(ctx context.Context) (*corev1.Secret, error) {
	secretList := &corev1.SecretList{}
	secretSelector := client.MatchingLabelsSelector{
		Selector: labels.NewSelector().
			Add(utils.MustNewRequirement(v1beta1constants.GardenRole, selection.Equals, v1beta1constants.GardenRoleGlobalMonitoring)).
			Add(utils.MustNewRequirement(v1beta1constants.GardenerPurpose, selection.DoesNotExist)),
	}
	if err := r.RuntimeClientSet.Client().List(ctx, secretList, client.InNamespace(r.GardenNamespace), secretSelector); err != nil {
		return nil, fmt.Errorf("failed to list global observability secrets in runtime cluster: %w", err)
	}
	...
	if len(secretList.Items) > 1 {
		...
		return nil, fmt.Errorf("there can be at most one global observability secret but found multiple: %s", strings.Join(secretNames, ", "))
	}
...
func (r *Reconciler) generateGlobalObservabilityIngressPassword(ctx context.Context, secretsManager secretsmanager.Interface) (*corev1.Secret, error) {
	var (
		...
		isManagedSecret = func(s *corev1.Secret) bool {
			return s.Labels[secretsmanager.LabelKeyManagedBy] == secretsmanager.LabelValueSecretsManager
		}
	)

	currentSecret, err := r.getGlobalObservabilitySecret(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to get global observability secret from runtime cluster: %w", err)
	}

	// If the password is manually generated by a human operator then do nothing.
	if currentSecret != nil && !isManagedSecret(currentSecret) {
		return currentSecret, nil
	}

	newSecret, err := secretsManager.Generate(ctx, &secretsutils.BasicAuthSecretConfig{
		Name:           secretName,
		Format:         secretsutils.BasicAuthFormatNormal,
		Username:       "admin",
		PasswordLength: 32,
	}, secretsmanager.Persist(), secretsmanager.Rotate(secretsmanager.InPlace), secretsmanager.WithLabels(map[string]string{
		v1beta1constants.GardenRole: v1beta1constants.GardenRoleGlobalMonitoring,
	}))
	if err != nil {
		return nil, fmt.Errorf("failed to generate global observability ingress secret: %w", err)
	}

	// Clean up the old secret if it was rotated.
	if currentSecret != nil && currentSecret.Name != newSecret.Name {
		if err := r.RuntimeClientSet.Client().Delete(ctx, currentSecret.DeepCopy()); client.IgnoreNotFound(err) != nil {
			return nil, fmt.Errorf("failed to delete old global monitoring secret %q from runtime cluster: %w", currentSecret.Name, err)
		}
	}
