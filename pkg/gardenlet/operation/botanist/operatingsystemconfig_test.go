...
var _ = Describe("operatingsystemconfig", func() {
	...
	Context("Operating System Config secrets", func() {
		var (
			...
			worker1Name = "worker1"
			worker1Key  = operatingsystemconfig.Key(semver.MustParse(kubernetesVersion), nil, &gardencorev1beta1.Worker{Name: worker1Name}, false, nil, nil)

			worker2Name                  = "worker2"
			worker2KubernetesVersion     = "4.5.6"
			worker2Key                   = operatingsystemconfig.Key(semver.MustParse(worker2KubernetesVersion), nil, &gardencorev1beta1.Worker{Name: worker2Name}, false, nil, nil)
