...
// GetCABundleFromCABundleSource resolves a CABundle to its PEM-encoded string content.
// Returns nil if the caBundle is nil or neither field is set.
func GetCABundleFromCABundleSource(ctx context.Context, c client.Client, caBundle *CABundle, namespace string) (*string, error) {
	if caBundle == nil {
		return nil, nil
	}

	if caBundle.Inline != nil {
		return caBundle.Inline, nil
	}

	if caBundle.SecretRef != nil {
		secret := &corev1.Secret{}
		if err := c.Get(ctx, client.ObjectKey{Namespace: namespace, Name: caBundle.SecretRef.Name}, secret); err != nil {
			return nil, fmt.Errorf("failed to get CA bundle secret %s/%s: %w", namespace, caBundle.SecretRef.Name, err)
		}

		caBundleData, ok := secret.Data[secretsutils.DataKeyCertificateBundle]
		if !ok {
			return nil, fmt.Errorf("CA bundle secret %s/%s is missing the data key %s", namespace, caBundle.SecretRef.Name, secretsutils.DataKeyCertificateBundle)
		}
		if len(caBundleData) == 0 {
			return nil, fmt.Errorf("CA bundle secret %s/%s has an empty CA bundle in the data key %s", namespace, caBundle.SecretRef.Name, secretsutils.DataKeyCertificateBundle)
		}

		return ptr.To(string(caBundleData)), nil
	}
...
func (v ImageVector) FindImage(name string, opts ...FindOptionFunc) (*Image, error) {
	o := (&FindOptions{}).ApplyOptions(opts)
