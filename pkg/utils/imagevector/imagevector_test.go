  ...
  - name: "%s"
    ...
    runtimeVersion: "%s"`, image1Src1.Name, *image1Src1.Repository, *image1Src1.Tag, *image1Src1.Tag, *image1Src1.RuntimeVersion)
		...
		Describe("#Read with caBundle", func() {
			...
			It("should read a vector with a secretRef caBundle", func() {
				vectorWithCA := `images:
  - name: "test"
...
caBundle:
  secretRef:
    name: my-ca-secret`
				...
				Expect(caBundle.SecretRef).NotTo(BeNil())
				Expect(caBundle.SecretRef.Name).To(Equal("my-ca-secret"))
			})
		})
		...
		DescribeTable("#MergeCABundle",
			...
			Entry("override wins over base",
				&CABundle{Inline: ptr.To("base")},
				&CABundle{Inline: ptr.To("override")},
				&CABundle{Inline: ptr.To("override")},
			),
