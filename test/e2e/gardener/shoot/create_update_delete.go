...
func verifyViewerKubeconfigShootAccess(s *ShootContext) {
	...
	It("Verify shoot access using viewer kubeconfig", func(ctx SpecContext) {
		Eventually(ctx, func(g Gomega) {
			...
			g.Expect(readOnlyShootClient.Client().Create(ctx, &corev1.ConfigMap{GenerateName: "test-", Namespace: metav1.NamespaceDefault})).To(BeForbiddenError())
			g.Expect(readOnlyShootClient.Client().Update(ctx, &corev1.ConfigMap{Name: "kube-root-ca.crt", Namespace: metav1.NamespaceDefault})).To(BeForbiddenError())
			g.Expect(readOnlyShootClient.Client().Patch(ctx, &corev1.ConfigMap{Name: "kube-root-ca.crt", Namespace: metav1.NamespaceDefault}, client.RawPatch(types.MergePatchType, []byte("{}")))).To(BeForbiddenError())
			g.Expect(readOnlyShootClient.Client().Delete(ctx, &corev1.ConfigMap{Name: "kube-root-ca.crt", Namespace: metav1.NamespaceDefault})).To(BeForbiddenError())
		}).Should(Succeed())
	}, SpecTimeout(time.Minute))
}
